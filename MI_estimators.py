...
class VIB(UpperWithPosterior):
    ...
    def update(self,mu,logvar):  # [nsample, 1]
        #print("使用了VIB...")
        kl_loss=1. / 2. * (mu ** 2 + logvar.exp() - 1. - logvar).mean()
...
class VIB_M(nn.Module):
    ...
    def update(self,mu,logvar):  # [nsample, 1]
        ...
        kl_loss = 1. / 2. * (mu ** 2 + logvar.exp() - 1. - logvar).mean()
        #kl_loss=torch.masked_select(kl_loss, features['span_masks'].unsqueeze(-1).bool()).mean()
