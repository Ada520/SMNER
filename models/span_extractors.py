...
class SpanExtractorWithSpanWidthEmbedding(SpanExtractor):
    ...
    def forward(
        ...
        where `embedded_span_size` depends on the way spans are represented.
        """
        # shape (batch_size, num_spans, embedding_dim),embedding_dim=input_dim*2,在这里是768*2
        #此时的span是span的起始表示词和结束表示词表示的的拼接
        ...
        if self._span_width_embedding is not None:
            ...
            span_embeddings = torch.cat([span_embeddings, span_width_embeddings], -1)#又拼接了span的长度表示
